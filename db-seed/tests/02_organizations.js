	...
	describe("SuperUser Actions", function() {
		...
		orgs.forEach(function (org) {
			...
			it(`SuperUser registers ${org.name}`, async function () {
				let result = {};
				try {
					result = await adminServer.organizations.create({ owner_user_id: adminData.user.id, ...org });
				}catch(e) {
					result = e.response;
				}
				assert.strictEqual(result.status, 201, `${org.name} was not added; StatusCode: ${result.status}`);
